...
ldnmp_install_nginx(){
	...
	if docker inspect "nginx" &>/dev/null; then
		if curl -sL "${github_proxy}github.com/honeok8s/conf/raw/refs/heads/main/nginx/ldnmp-nginx-docker-compose.yml" | head -n 19 | diff - "/data/docker_data/web/docker-compose.yml" &>/dev/null; then
