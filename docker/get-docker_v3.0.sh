...
gitdocker_version="2.0"

os_release=$(grep '^ID=' /etc/os-release | cut -d'=' -f2 | tr -d '"' | tr '[:upper:]' '[:lower:]')
support_os_release=$(grep '^PRETTY_NAME=' /etc/os-release | cut -d '"' -f 2)
...
EOF
	...
	_blue "Version: v$gitdocker_version"
	_purple "Github: https://github.com/honeok8s"
}
