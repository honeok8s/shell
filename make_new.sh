...
backup_current_make() {
    ...
    if command -v make &> /dev/null 2>&1; then
        ...
        printf "${yellow}检测到现有的make版本 (${make_version}), 正在备份.${white}\n"
...
restore_backup_make() {
    ...
    if [ -d "$backup_bin_dir" ]; then
        ...
        if [ -f "$backup_bin_dir/make" ]; then
            ...
            \cp -fr "$backup_bin_dir/make" "$make_path"
...
install_make() {
    ...
    if [ -z "$make_version" ]; then
        printf "${red}未选择有效的 make 版本，安装取消。${white}\n"
        return 1
    fi
...
uninstall_make() {
    local make_version
    local make_source_dir

    make_version=$(get_make_version)
    make_source_dir="${SOFTWARE_DIR}/make-${make_version}"
